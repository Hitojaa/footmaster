export type QuizCategory = 'joueurs' | 'clubs' | 'competitions' | 'histoire' | 'coachs';
export type Difficulty = 'facile' | 'moyen' | 'difficile';

export interface QuizQuestion {
  id: string;
  question: string;
  options: string[];
  correctAnswer: number;
  explanation: string;
  emoji: string;
  difficulty: Difficulty;
}

export interface Quiz {
  id: string;
  title: string;
  subtitle: string;
  category: QuizCategory;
  emoji: string;
  bgGradient: string;
  difficulty: Difficulty;
  timePerQuestion: number;
  questions: QuizQuestion[];
  reward: string;
  badge: string;
}

export const quizzes: Quiz[] = [
  {
    id: 'legendes',
    title: 'Quiz Légendes',
    subtitle: 'Teste tes connaissances sur les plus grands joueurs de l\'histoire',
    category: 'joueurs',
    emoji: '🐐',
    bgGradient: 'from-yellow-900 via-amber-800 to-orange-800',
    difficulty: 'moyen',
    timePerQuestion: 20,
    badge: '🏅 Badge Historien',
    reward: '500 XP',
    questions: [
      {
        id: 'l1',
        question: 'Combien de Ballons d\'Or Lionel Messi a-t-il remporté ?',
        options: ['5', '6', '7', '8'],
        correctAnswer: 3,
        explanation: 'Messi a remporté 8 Ballons d\'Or (2009, 2010, 2011, 2012, 2015, 2019, 2021, 2023), un record absolu.',
        emoji: '🏅',
        difficulty: 'facile',
      },
      {
        id: 'l2',
        question: 'Quel est le surnom de Ronaldo Nazário ?',
        options: ['Le Phénomène', 'O Rei', 'CR7', 'La Pulga'],
        correctAnswer: 0,
        explanation: '"O Fenômeno" (Le Phénomène) est le surnom de Ronaldo Nazário, en référence à ses capacités physiques et techniques hors du commun.',
        emoji: '🌪️',
        difficulty: 'facile',
      },
      {
        id: 'l3',
        question: 'En quelle année Zinédine Zidane a-t-il remporté la Coupe du Monde ?',
        options: ['1994', '1996', '1998', '2000'],
        correctAnswer: 2,
        explanation: 'La France a remporté la Coupe du Monde 1998 organisée sur son sol. Zidane a marqué deux buts de la tête en finale contre le Brésil (3-0).',
        emoji: '🇫🇷',
        difficulty: 'facile',
      },
      {
        id: 'l4',
        question: 'Combien de Coupes du Monde Pelé a-t-il remportées ?',
        options: ['1', '2', '3', '4'],
        correctAnswer: 2,
        explanation: 'Pelé a remporté 3 Coupes du Monde (1958, 1962, 1970) — un record qu\'aucun autre joueur n\'a égalé.',
        emoji: '⭐',
        difficulty: 'facile',
      },
      {
        id: 'l5',
        question: 'Quel joueur a marqué un hat-trick en finale de la Coupe du Monde 2022 ?',
        options: ['Lionel Messi', 'Kylian Mbappé', 'Olivier Giroud', 'Antoine Griezmann'],
        correctAnswer: 1,
        explanation: 'Kylian Mbappé a marqué 3 buts en finale (74e, 80e, 118e minutes) — la France perd quand même aux tirs au but contre l\'Argentine.',
        emoji: '⚡',
        difficulty: 'moyen',
      },
      {
        id: 'l6',
        question: 'Quel est le record de buts en une saison de calendrier de Messi ?',
        options: ['73 buts', '83 buts', '91 buts', '100 buts'],
        correctAnswer: 2,
        explanation: 'Messi a marqué 91 buts en 2012 (année civile), pulvérisant l\'ancien record de Gerd Müller (85 buts en 1972). Un record qui semble imbattable.',
        emoji: '🎯',
        difficulty: 'difficile',
      },
      {
        id: 'l7',
        question: 'Dans quel club Zidane a-t-il fait ses débuts professionnels ?',
        options: ['Olympique de Marseille', 'Girondins de Bordeaux', 'AS Cannes', 'Juventus'],
        correctAnswer: 2,
        explanation: 'Zidane a fait ses débuts à l\'AS Cannes à 16 ans en 1988, avant de rejoindre Bordeaux puis la Juventus et le Real Madrid.',
        emoji: '🎭',
        difficulty: 'difficile',
      },
      {
        id: 'l8',
        question: 'Quelle est la vitesse de pointe approximative de Kylian Mbappé ?',
        options: ['30 km/h', '33 km/h', '36 km/h', '39 km/h'],
        correctAnswer: 2,
        explanation: 'Mbappé a été mesuré à plus de 36 km/h sur le terrain, ce qui est comparable à la vitesse de sprint d\'un athlète de haut niveau.',
        emoji: '⚡',
        difficulty: 'moyen',
      },
      {
        id: 'l9',
        question: 'Combien de buts Cristiano Ronaldo a-t-il marqués en Ligue des Champions ?',
        options: ['100 buts', '120 buts', '140 buts', '160 buts'],
        correctAnswer: 2,
        explanation: 'Cristiano Ronaldo est le meilleur buteur de l\'histoire de la Ligue des Champions avec 140 buts, loin devant tous les autres.',
        emoji: '👑',
        difficulty: 'moyen',
      },
      {
        id: 'l10',
        question: 'Combien de buts Haaland a-t-il inscrits avec Manchester City toutes compétitions confondues en 2022-23 ?',
        options: ['38 buts', '44 buts', '52 buts', '60 buts'],
        correctAnswer: 2,
        explanation: 'Haaland a marqué 52 buts toutes compétitions confondues en 2022-23 (36 en Premier League + 7 en LDC + 5 en FA Cup + 4 autres) — un record historique pour une seule saison.',
        emoji: '🤖',
        difficulty: 'difficile',
      },
    ],
  },
  {
    id: 'coupe-du-monde',
    title: 'Quiz Coupe du Monde',
    subtitle: 'Maîtrises-tu l\'histoire du plus grand tournoi du monde ?',
    category: 'competitions',
    emoji: '🌍',
    bgGradient: 'from-green-900 via-teal-800 to-emerald-800',
    difficulty: 'moyen',
    timePerQuestion: 20,
    badge: '🌍 Badge Mondialiste',
    reward: '600 XP',
    questions: [
      {
        id: 'cdm1',
        question: 'Quelle nation a remporté le plus de Coupes du Monde ?',
        options: ['Allemagne', 'Italie', 'Brésil', 'Argentine'],
        correctAnswer: 2,
        explanation: 'Le Brésil est la nation la plus titrée avec 5 titres (1958, 1962, 1970, 1994, 2002). L\'Allemagne et l\'Italie suivent avec 4 titres chacune.',
        emoji: '🇧🇷',
        difficulty: 'facile',
      },
      {
        id: 'cdm2',
        question: 'Où s\'est jouée la première Coupe du Monde en 1930 ?',
        options: ['Brésil', 'Argentine', 'Uruguay', 'Italie'],
        correctAnswer: 2,
        explanation: 'L\'Uruguay a organisé et remporté la première Coupe du Monde en 1930. L\'Argentine fut finaliste, battue 4-2 en finale.',
        emoji: '🇺🇾',
        difficulty: 'moyen',
      },
      {
        id: 'cdm3',
        question: 'Qui est le meilleur buteur de l\'histoire de la Coupe du Monde ?',
        options: ['Ronaldo Nazário', 'Just Fontaine', 'Miroslav Klose', 'Pelé'],
        correctAnswer: 2,
        explanation: 'Miroslav Klose (Allemagne) est le meilleur buteur de l\'histoire de la Coupe du Monde avec 16 buts (2002, 2006, 2010, 2014).',
        emoji: '🇩🇪',
        difficulty: 'moyen',
      },
      {
        id: 'cdm4',
        question: 'Quel pays a réalisé le "Miracle de Berne" en 1954 ?',
        options: ['Hongrie', 'Allemagne de l\'Ouest', 'France', 'Yougoslavie'],
        correctAnswer: 1,
        explanation: 'L\'Allemagne de l\'Ouest a battu la Hongrie 3-2 en finale 1954 alors que la Hongrie était favorite et n\'avait pas perdu depuis 4 ans.',
        emoji: '🇩🇪',
        difficulty: 'difficile',
      },
      {
        id: 'cdm5',
        question: 'Combien de fois la France a-t-elle remporté la Coupe du Monde ?',
        options: ['1', '2', '3', '4'],
        correctAnswer: 1,
        explanation: 'La France a remporté 2 Coupes du Monde : 1998 sur son sol (3-0 vs Brésil) et 2018 en Russie (4-2 vs Croatie).',
        emoji: '🇫🇷',
        difficulty: 'facile',
      },
      {
        id: 'cdm6',
        question: 'Quelle est la plus grande victoire de l\'histoire de la Coupe du Monde ?',
        options: ['Hongrie 10-1 El Salvador (1982)', 'Allemagne 8-0 Arabie Saoudite (2002)', 'Yougoslavie 9-0 Zaïre (1974)', 'Allemagne 7-1 Brésil (2014)'],
        correctAnswer: 0,
        explanation: 'La Hongrie a battu El Salvador 10-1 en 1982. C\'est la victoire la plus large de l\'histoire de la Coupe du Monde en phase de groupes.',
        emoji: '😱',
        difficulty: 'difficile',
      },
      {
        id: 'cdm7',
        question: 'Quel joueur a été sacré Meilleur Joueur du Mondial 2022 ?',
        options: ['Kylian Mbappé', 'Luka Modric', 'Lionel Messi', 'Emiliano Martinez'],
        correctAnswer: 2,
        explanation: 'Lionel Messi a remporté le Ballon d\'Or du Mondial 2022 — sa deuxième fois (après 2014) — en plus du titre de champion du monde avec l\'Argentine.',
        emoji: '🐐',
        difficulty: 'facile',
      },
      {
        id: 'cdm8',
        question: 'Quel est le stade qui a accueilli la finale du Mondial 2022 ?',
        options: ['Al Bayt Stadium', 'Ahmad Bin Ali Stadium', 'Al Janoub Stadium', 'Lusail Stadium'],
        correctAnswer: 3,
        explanation: 'Le Lusail Iconic Stadium (89 000 places) a accueilli la finale Argentine-France le 18 décembre 2022. C\'est le plus grand stade du Qatar.',
        emoji: '🏟️',
        difficulty: 'difficile',
      },
      {
        id: 'cdm9',
        question: 'La "Manita" (5-0) de l\'Espagne vs quelle nation en 2010 marque les demi-finales ?',
        options: ['Allemagne', 'Paraguay', 'Uruguay', 'Brésil'],
        correctAnswer: 0,
        explanation: 'L\'Espagne a battu l\'Allemagne 1-0 en demi-finale du Mondial 2010 — le seul but de Puyol de la tête. L\'Espagne a ensuite battu les Pays-Bas 1-0 en finale (Torres).',
        emoji: '🇪🇸',
        difficulty: 'difficile',
      },
      {
        id: 'cdm10',
        question: 'Quel entraîneur a mené le Brésil à la Coupe du Monde 1970 (considérée comme le meilleur Brésil de l\'histoire) ?',
        options: ['Mário Zagallo', 'Telê Santana', 'Luiz Felipe Scolari', 'Vicente Feola'],
        correctAnswer: 0,
        explanation: 'Mário Zagallo était l\'entraîneur du Brésil en 1970. Il avait également joué (et gagné) les éditions 1958 et 1962 — devenant le premier homme à gagner la CdM comme joueur et entraîneur.',
        emoji: '🇧🇷',
        difficulty: 'difficile',
      },
    ],
  },
  {
    id: 'ldc',
    title: 'Quiz Ligue des Champions',
    subtitle: 'L\'hymne résonne — es-tu prêt à affronter la plus grande compétition de clubs ?',
    category: 'competitions',
    emoji: '⭐',
    bgGradient: 'from-blue-900 via-indigo-800 to-purple-900',
    difficulty: 'difficile',
    timePerQuestion: 20,
    badge: '⭐ Badge Expert LDC',
    reward: '800 XP',
    questions: [
      {
        id: 'ldc1',
        question: 'Quel club a remporté le plus de Ligues des Champions ?',
        options: ['Bayern Munich', 'AC Milan', 'Liverpool', 'Real Madrid'],
        correctAnswer: 3,
        explanation: 'Le Real Madrid est le club le plus titré de la LDC avec 15 titres (1956, 1957, 1958, 1959, 1960, 1966, 1998, 2000, 2002, 2014, 2016, 2017, 2018, 2022, 2024).',
        emoji: '👑',
        difficulty: 'facile',
      },
      {
        id: 'ldc2',
        question: 'En quelle année a eu lieu le "Miracle d\'Istanbul" (remontée de Liverpool vs AC Milan) ?',
        options: ['2003', '2005', '2007', '2009'],
        correctAnswer: 1,
        explanation: 'Le "Miracle d\'Istanbul" s\'est produit le 25 mai 2005. Liverpool, mené 3-0 à la mi-temps, a égalisé avant de gagner aux tirs au but.',
        emoji: '🔴',
        difficulty: 'facile',
      },
      {
        id: 'ldc3',
        question: 'Qui a marqué le but de la victoire de Man United en 1999 dans le temps additionnel ?',
        options: ['Andy Cole', 'Ole Gunnar Solskjaer', 'Teddy Sheringham', 'Peter Schmeichel'],
        correctAnswer: 1,
        explanation: 'Ole Gunnar Solskjaer a marqué à la 93e minute pour offrir à Man United la LDC 1999 contre le Bayern Munich (2-1). Sheringham avait égalisé à la 91e.',
        emoji: '😱',
        difficulty: 'moyen',
      },
      {
        id: 'ldc4',
        question: 'Quelle est la volée iconique de Zidane en finale 2002 contre le Bayer Leverkusen ?',
        options: ['Du pied droit de l\'intérieur', 'Du pied gauche à la retourné', 'Du pied gauche en volée de loin', 'Tête piquée'],
        correctAnswer: 2,
        explanation: 'Zidane a marqué d\'une volée sublime du pied gauche à 25 mètres — considérée comme l\'un des plus beaux buts de l\'histoire de la LDC. Et Zidane est droitier.',
        emoji: '🎨',
        difficulty: 'moyen',
      },
      {
        id: 'ldc5',
        question: 'Le Barça a-t-il réalisé "La Remontada" contre quelle équipe en 2017 ?',
        options: ['Chelsea', 'Manchester City', 'PSG', 'Arsenal'],
        correctAnswer: 2,
        explanation: 'Le FC Barcelone a remporté 6-1 le match retour contre le PSG après avoir perdu 4-0 à l\'aller. Sergi Roberto a marqué le but de la qualification à la 95e minute.',
        emoji: '🔥',
        difficulty: 'facile',
      },
      {
        id: 'ldc6',
        question: 'Combien de fois Cristiano Ronaldo a-t-il remporté la LDC ?',
        options: ['3', '4', '5', '6'],
        correctAnswer: 2,
        explanation: 'CR7 a remporté 5 LDC : 1 avec Manchester United (2008) et 4 avec le Real Madrid (2014, 2016, 2017, 2018) — un record personnel.',
        emoji: '👑',
        difficulty: 'moyen',
      },
      {
        id: 'ldc7',
        question: 'Quelle équipe a Chelsea battue dans leur propre stade en 2012 ?',
        options: ['FC Barcelone', 'Real Madrid', 'Bayern Munich', 'Juventus'],
        correctAnswer: 2,
        explanation: 'Chelsea a battu le Bayern Munich aux tirs au but dans l\'Allianz Arena de Munich en 2012 — victoire impensable que Didier Drogba a portée avec un but égalisateur tardif.',
        emoji: '🔵',
        difficulty: 'difficile',
      },
      {
        id: 'ldc8',
        question: 'Quel attaquant a marqué 5 buts en 21 minutes pour Dortmund en LDC ?',
        options: ['Robert Lewandowski', 'Pierre-Emerick Aubameyang', 'Erling Haaland', 'Marco Reus'],
        correctAnswer: 2,
        explanation: 'Haaland a marqué 5 buts en 21 minutes en entrant en jeu contre Séville lors de la LDC 2020-21 — une performance extraordinaire pour un joueur de 20 ans.',
        emoji: '💥',
        difficulty: 'difficile',
      },
      {
        id: 'ldc9',
        question: 'En quelle année l\'Ajax a-t-il remporté la Coupe des Clubs Champions pour la première fois ?',
        options: ['1967', '1969', '1971', '1973'],
        correctAnswer: 2,
        explanation: 'L\'Ajax d\'Amsterdam a remporté la Coupe des clubs champions pour la première fois en 1971, début de son triplé historique (1971, 1972, 1973) sous la direction de Stefan Kovacs.',
        emoji: '🇳🇱',
        difficulty: 'difficile',
      },
      {
        id: 'ldc10',
        question: 'Quelle est la victoire la plus large de l\'histoire de la LDC ?',
        options: ['Real Madrid 8-0 Malmö', 'Liverpool 8-0 Besiktas', 'Borussia Dortmund 8-4 Legia Warszawa', 'Bayern Munich 8-2 Barcelone'],
        correctAnswer: 3,
        explanation: 'Le Bayern Munich a battu le FC Barcelone 8-2 le 14 août 2020 lors d\'un quart de finale en match unique à Lisbonne — une humiliation historique pour le Barça.',
        emoji: '😱',
        difficulty: 'difficile',
      },
    ],
  },
  {
    id: 'football-2026',
    title: 'Quiz Football 2026',
    subtitle: 'Nouvelles stars, clubs modernes et faits récents jusqu\'à 2026',
    category: 'histoire',
    emoji: '🚀',
    bgGradient: 'from-slate-900 via-indigo-900 to-blue-900',
    difficulty: 'moyen',
    timePerQuestion: 20,
    badge: '🚀 Badge 2026',
    reward: '900 XP',
    questions: [
      {
        id: 'q26-1',
        question: 'Quel club a remporté sa première Ligue des Champions en 2023 ?',
        options: ['Arsenal', 'Manchester City', 'PSG', 'Atlético de Madrid'],
        correctAnswer: 1,
        explanation: 'Manchester City a remporté sa première C1 le 10 juin 2023 en battant l\'Inter 1-0.',
        emoji: '⭐',
        difficulty: 'facile',
      },
      {
        id: 'q26-2',
        question: 'Quel jeune ailier espagnol du Barça est déjà titulaire en sélection en 2026 ?',
        options: ['Nico Williams', 'Pedri', 'Lamine Yamal', 'Ferran Torres'],
        correctAnswer: 2,
        explanation: 'Lamine Yamal est considéré comme l\'un des plus grands talents de sa génération.',
        emoji: '🌟',
        difficulty: 'facile',
      },
      {
        id: 'q26-3',
        question: 'Quel entraîneur a mené Leverkusen à une saison invaincue en Bundesliga 2023-24 ?',
        options: ['Julian Nagelsmann', 'Xabi Alonso', 'Thomas Tuchel', 'Edin Terzić'],
        correctAnswer: 1,
        explanation: 'Xabi Alonso a signé une saison historique avec Leverkusen.',
        emoji: '🧠',
        difficulty: 'moyen',
      },
      {
        id: 'q26-4',
        question: 'Qui a marqué le but décisif de la finale de Ligue des Champions 2023 ?',
        options: ['Haaland', 'Foden', 'Rodri', 'Bernardo Silva'],
        correctAnswer: 2,
        explanation: 'Rodri a inscrit l\'unique but de City-Inter (1-0).',
        emoji: '🎯',
        difficulty: 'moyen',
      },
      {
        id: 'q26-5',
        question: 'Quel club a remporté la Ligue des Champions 2024 ?',
        options: ['Borussia Dortmund', 'Real Madrid', 'Bayern Munich', 'Inter Milan'],
        correctAnswer: 1,
        explanation: 'Le Real Madrid a gagné la finale 2024 à Wembley contre Dortmund (2-0).',
        emoji: '👑',
        difficulty: 'facile',
      },
      {
        id: 'q26-6',
        question: 'Quel milieu espagnol de Manchester City est considéré comme un maître du poste de n°6 ?',
        options: ['Gavi', 'Pedri', 'Rodri', 'Fabián Ruiz'],
        correctAnswer: 2,
        explanation: 'Rodri est devenu une référence mondiale du milieu défensif.',
        emoji: '🛡️',
        difficulty: 'facile',
      },
      {
        id: 'q26-7',
        question: 'Quel est le record de buts de Haaland en Premier League sur une saison à 38 matchs ?',
        options: ['34', '35', '36', '37'],
        correctAnswer: 2,
        explanation: 'Haaland a marqué 36 buts en 2022-23, record de Premier League sur format 38 journées.',
        emoji: '🤖',
        difficulty: 'moyen',
      },
      {
        id: 'q26-8',
        question: 'Quel club londonien est entraîné par Mikel Arteta en 2026 ?',
        options: ['Chelsea', 'Arsenal', 'Tottenham', 'West Ham'],
        correctAnswer: 1,
        explanation: 'Arteta dirige Arsenal et a replacé le club parmi les prétendants au titre.',
        emoji: '📋',
        difficulty: 'facile',
      },
      {
        id: 'q26-9',
        question: 'Quel joueur anglais porte régulièrement le brassard créatif du Real Madrid depuis 2023 ?',
        options: ['Harry Kane', 'Phil Foden', 'Cole Palmer', 'Jude Bellingham'],
        correctAnswer: 3,
        explanation: 'Jude Bellingham s\'est imposé rapidement comme leader technique au Real.',
        emoji: '🦁',
        difficulty: 'moyen',
      },
      {
        id: 'q26-10',
        question: 'Quel sélectionneur a mené la France au titre mondial 2018 et à la finale 2022 ?',
        options: ['Laurent Blanc', 'Didier Deschamps', 'Raymond Domenech', 'Hervé Renard'],
        correctAnswer: 1,
        explanation: 'Didier Deschamps a remporté la Coupe du Monde 2018 et atteint la finale en 2022.',
        emoji: '🇫🇷',
        difficulty: 'facile',
      },
    ],
  },
  {
    id: 'quiz-clubs-elite',
    title: 'Quiz Clubs Elite',
    subtitle: 'Tu connais les grands clubs et leurs rivalités ?',
    category: 'clubs',
    emoji: '🏟️',
    bgGradient: 'from-emerald-900 via-teal-800 to-cyan-800',
    difficulty: 'moyen',
    timePerQuestion: 20,
    badge: '🏟️ Badge Club Master',
    reward: '700 XP',
    questions: [
      {
        id: 'club-1',
        question: 'Quel stade est le domicile de Liverpool ?',
        options: ['Old Trafford', 'Anfield', 'Emirates Stadium', 'Etihad Stadium'],
        correctAnswer: 1,
        explanation: 'Liverpool joue à Anfield, stade iconique des Reds.',
        emoji: '🔴',
        difficulty: 'facile',
      },
      {
        id: 'club-2',
        question: 'Quel derby oppose Arsenal à Tottenham ?',
        options: ['Merseyside Derby', 'Derby della Madonnina', 'North London Derby', 'Manchester Derby'],
        correctAnswer: 2,
        explanation: 'Arsenal-Tottenham est le North London Derby.',
        emoji: '🔥',
        difficulty: 'facile',
      },
      {
        id: 'club-3',
        question: 'Quel club italien est surnommé \"La Vecchia Signora\" ?',
        options: ['AC Milan', 'Inter', 'Napoli', 'Juventus'],
        correctAnswer: 3,
        explanation: 'La Juventus est historiquement appelée \"La Vecchia Signora\".',
        emoji: '⚪⚫',
        difficulty: 'moyen',
      },
      {
        id: 'club-4',
        question: 'Quel club est le seul jamais relégué de Serie A ?',
        options: ['Inter Milan', 'Roma', 'Lazio', 'Fiorentina'],
        correctAnswer: 0,
        explanation: 'L\'Inter est le seul club italien à n\'avoir jamais été relégué de Serie A.',
        emoji: '🔵⚫',
        difficulty: 'difficile',
      },
      {
        id: 'club-5',
        question: 'Quel club a gagné 6 Ligues des Champions en Angleterre ?',
        options: ['Manchester United', 'Liverpool', 'Chelsea', 'Arsenal'],
        correctAnswer: 1,
        explanation: 'Liverpool compte 6 titres européens majeurs en C1/LDC.',
        emoji: '🏆',
        difficulty: 'moyen',
      },
    ],
  },
  {
    id: 'nouvelles-etoiles',
    title: 'Quiz Nouvelles Étoiles',
    subtitle: 'Des talents émergents aux nouvelles stars — tu connais les visages du foot actuel ?',
    category: 'joueurs',
    emoji: '🌟',
    bgGradient: 'from-violet-900 via-purple-800 to-fuchsia-800',
    difficulty: 'moyen',
    timePerQuestion: 20,
    badge: '🌟 Badge Next Gen',
    reward: '750 XP',
    questions: [
      {
        id: 'ng1',
        question: 'Quel joueur anglais formé à Manchester City a explosé à Chelsea en 2023-24 ?',
        options: ['Marcus Rashford', 'Phil Foden', 'Cole Palmer', 'Anthony Gordon'],
        correctAnswer: 2,
        explanation: 'Cole Palmer a épaté toute l\'Angleterre avec 22 buts et 11 passes déc. lors de sa première saison pleine à Chelsea (2023-24), remportant le titre de meilleur jeune joueur de PL.',
        emoji: '🧊',
        difficulty: 'facile',
      },
      {
        id: 'ng2',
        question: 'Lors de quelle saison Bayer Leverkusen a-t-il remporté son premier titre de Bundesliga sans perdre un seul match ?',
        options: ['2022-23', '2023-24', '2024-25', '2021-22'],
        correctAnswer: 1,
        explanation: 'Sous Xabi Alonso, Leverkusen a réalisé une saison historique en 2023-24 : invaincu en championnat tout au long de la saison, un exploit rarissime dans le football européen.',
        emoji: '🏆',
        difficulty: 'facile',
      },
      {
        id: 'ng3',
        question: 'Quel record Mohamed Salah a-t-il établi en Premier League lors de la saison 2017-18 ?',
        options: ['25 buts', '28 buts', '32 buts', '35 buts'],
        correctAnswer: 2,
        explanation: 'Mohamed Salah a inscrit 32 buts en Premier League en 2017-18 lors d\'une saison à 38 journées — un record absolu pour ce format qui n\'a jamais été égalé.',
        emoji: '👑',
        difficulty: 'moyen',
      },
      {
        id: 'ng4',
        question: 'Avec quel club Ronaldinho a-t-il remporté la Ligue des Champions ?',
        options: ['PSG', 'Santos', 'FC Barcelone', 'AC Milan'],
        correctAnswer: 2,
        explanation: 'Ronaldinho a remporté la LDC 2006 avec le FC Barcelone en battant Arsenal 2-1 en finale à Paris. Il était à son sommet, double Ballon d\'Or (2004-2005).',
        emoji: '🎭',
        difficulty: 'facile',
      },
      {
        id: 'ng5',
        question: 'Thierry Henry est le meilleur buteur historique de quel club anglais ?',
        options: ['Liverpool', 'Chelsea', 'Arsenal', 'Manchester City'],
        correctAnswer: 2,
        explanation: 'Thierry Henry est le meilleur buteur historique d\'Arsenal avec 228 buts. Il reste l\'une des légendes absolues du club et de la Premier League.',
        emoji: '💨',
        difficulty: 'facile',
      },
      {
        id: 'ng6',
        question: 'Diego Maradona a remporté la Coupe du Monde avec l\'Argentine lors de quelle édition ?',
        options: ['1982', '1986', '1990', '1994'],
        correctAnswer: 1,
        explanation: 'Maradona a mené l\'Argentine au titre mondial 1986 au Mexique, avec notamment le "But du Siècle" contre l\'Angleterre en quart de finale et deux buts decisifs en demi-finale contre la Belgique.',
        emoji: '🌟',
        difficulty: 'facile',
      },
      {
        id: 'ng7',
        question: 'Robert Lewandowski a rejoint le FC Barcelone en provenance de quel club en 2022 ?',
        options: ['Borussia Dortmund', 'Juventus', 'Bayern Munich', 'RB Leipzig'],
        correctAnswer: 2,
        explanation: 'Lewandowski a quitté le Bayern Munich après 8 saisons et des dizaines de titres pour rejoindre le FC Barcelone à l\'été 2022, à 33 ans, prouvant qu\'il pouvait encore dominer.',
        emoji: '🤖',
        difficulty: 'moyen',
      },
      {
        id: 'ng8',
        question: 'Phil Foden a passé toute sa carrière professionnelle dans quel club jusqu\'en 2025 ?',
        options: ['Manchester United', 'Liverpool', 'Arsenal', 'Manchester City'],
        correctAnswer: 3,
        explanation: 'Phil Foden est un pur produit de la City Football Academy. Depuis ses débuts en 2017 jusqu\'en 2025, il n\'a jamais joué dans un autre club pro, remportant 5 PL et une LDC.',
        emoji: '🎻',
        difficulty: 'facile',
      },
      {
        id: 'ng9',
        question: 'Quel trophée Gavi a-t-il remporté avec l\'Espagne en été 2024 ?',
        options: ['Nations League', 'Coupe du Monde U21', 'Euro', 'Copa América'],
        correctAnswer: 2,
        explanation: 'Gavi a remporté l\'Euro 2024 avec l\'Espagne, qui a dominé tout le tournoi en Allemagne et battu l\'Angleterre 2-1 en finale grâce à un but de Mikel Oyarzabal.',
        emoji: '🔥',
        difficulty: 'moyen',
      },
      {
        id: 'ng10',
        question: 'Quel est le surnom de Florian Wirtz, la star allemande de Bayer Leverkusen ?',
        options: ['Der Maschine', 'Flo Magic', 'The Wizard', 'Wirtzwind'],
        correctAnswer: 3,
        explanation: '"Wirtzwind" (jeu de mots entre Wirtz et Wirbelwind = tourbillon en allemand) est son surnom en Allemagne, en référence à sa capacité à déstabiliser les défenses à toute vitesse.',
        emoji: '⚡',
        difficulty: 'difficile',
      },
    ],
  },

  {
    id: 'nouvelles-etoiles',
    title: 'Quiz Nouvelles Étoiles',
    subtitle: "Des talents émergents aux nouvelles stars — tu connais les visages du foot actuel ?",
    category: 'joueurs',
    emoji: '🌟',
    bgGradient: 'from-violet-900 via-purple-800 to-fuchsia-800',
    difficulty: 'moyen',
    timePerQuestion: 20,
    badge: '🌟 Badge Next Gen',
    reward: '750 XP',
    questions: [
      {
        id: 'ng1',
        question: "Quel joueur anglais formé à Manchester City a explosé à Chelsea lors de la saison 2023-24 ?",
        options: ['Marcus Rashford', 'Phil Foden', 'Cole Palmer', 'Anthony Gordon'],
        correctAnswer: 2,
        explanation: "Cole Palmer a épaté toute l'Angleterre avec 22 buts et 11 passes déc. lors de sa première saison pleine à Chelsea (2023-24), remportant le titre de meilleur jeune joueur de PL.",
        emoji: '🧊',
        difficulty: 'facile',
      },
      {
        id: 'ng2',
        question: 'Lors de quelle saison Bayer Leverkusen a-t-il remporté son premier titre de Bundesliga sans perdre un seul match ?',
        options: ['2022-23', '2023-24', '2024-25', '2021-22'],
        correctAnswer: 1,
        explanation: 'Sous Xabi Alonso, Leverkusen a réalisé une saison historique en 2023-24 : invaincu en championnat tout au long de la saison, un exploit rarissime dans le football européen.',
        emoji: '🏆',
        difficulty: 'facile',
      },
      {
        id: 'ng3',
        question: 'Quel record Mohamed Salah a-t-il établi en Premier League lors de la saison 2017-18 ?',
        options: ['25 buts', '28 buts', '32 buts', '35 buts'],
        correctAnswer: 2,
        explanation: 'Mohamed Salah a inscrit 32 buts en Premier League en 2017-18 lors de la saison à 38 journées — un record absolu pour ce format.',
        emoji: '👑',
        difficulty: 'moyen',
      },
      {
        id: 'ng4',
        question: "Avec quel club Ronaldinho a-t-il remporté la Ligue des Champions ?",
        options: ['PSG', 'Santos', 'FC Barcelone', 'AC Milan'],
        correctAnswer: 2,
        explanation: "Ronaldinho a remporté la LDC 2006 avec le FC Barcelone en battant Arsenal 2-1 en finale à Paris. Il était au sommet de son art, double Ballon d'Or (2004-2005).",
        emoji: '🎭',
        difficulty: 'facile',
      },
      {
        id: 'ng5',
        question: "Thierry Henry est le meilleur buteur historique de quel club anglais ?",
        options: ['Liverpool', 'Chelsea', 'Arsenal', 'Manchester City'],
        correctAnswer: 2,
        explanation: "Thierry Henry est le meilleur buteur historique d'Arsenal avec 228 buts. Il reste l'une des légendes absolues du club et de la Premier League.",
        emoji: '💨',
        difficulty: 'facile',
      },
      {
        id: 'ng6',
        question: "Diego Maradona a remporté la Coupe du Monde avec l'Argentine lors de quelle édition ?",
        options: ['1982', '1986', '1990', '1994'],
        correctAnswer: 1,
        explanation: "Maradona a mené l'Argentine au titre mondial 1986 au Mexique, avec le célèbre 'But du Siècle' contre l'Angleterre en quart de finale.",
        emoji: '🌟',
        difficulty: 'facile',
      },
      {
        id: 'ng7',
        question: 'Robert Lewandowski a rejoint le FC Barcelone en provenance de quel club en 2022 ?',
        options: ['Borussia Dortmund', 'Juventus', 'Bayern Munich', 'RB Leipzig'],
        correctAnswer: 2,
        explanation: "Lewandowski a quitté le Bayern Munich après 8 saisons et des dizaines de titres pour rejoindre le FC Barcelone à l'été 2022.",
        emoji: '🤖',
        difficulty: 'moyen',
      },
      {
        id: 'ng8',
        question: "Phil Foden a passé toute sa carrière professionnelle dans quel club jusqu'en 2025 ?",
        options: ['Manchester United', 'Liverpool', 'Arsenal', 'Manchester City'],
        correctAnswer: 3,
        explanation: "Phil Foden est un pur produit de la City Football Academy. Depuis ses débuts en 2017 jusqu'en 2025, il n'a jamais joué dans un autre club pro, remportant 5 PL et une LDC.",
        emoji: '🎻',
        difficulty: 'facile',
      },
      {
        id: 'ng9',
        question: "Quel trophée Gavi a-t-il remporté avec l'Espagne en été 2024 ?",
        options: ['Nations League', 'Coupe du Monde U21', 'Euro', 'Copa América'],
        correctAnswer: 2,
        explanation: "Gavi a remporté l'Euro 2024 avec l'Espagne, qui a dominé tout le tournoi en Allemagne et battu l'Angleterre 2-1 en finale.",
        emoji: '🔥',
        difficulty: 'moyen',
      },
      {
        id: 'ng10',
        question: "Quel Ballon d'Or Rodri a-t-il remporté en 2024, une première pour son poste depuis des décennies ?",
        options: ['FIFA Best', "Ballon d'Or", "Soulier d'Or", 'UEFA Player of the Year'],
        correctAnswer: 1,
        explanation: "Rodri a remporté le Ballon d'Or 2024, devenant le premier milieu défensif pur à recevoir cette récompense depuis Matthias Sammer en 1996.",
        emoji: '🧩',
        difficulty: 'moyen',
      },
    ],
  },
];

export const getQuizById = (id: string) => quizzes.find(q => q.id === id);
